...
if not model_saved:
    ...
    datagen = ImageDataGenerator(
        ...
        vertical_flip=False,
        preprocessing_function=image_processing,
        fill_mode="nearest")
    ...
    plt.savefig('vgg_contrast_loss.png')
    plt.clf()
    ...
    plt.savefig('vgg_contrast_accuracy.png')
    plt.clf()
    model.save('model_vgg_contrast')
    ...
    # Print the dataframe
    print(df_vgg_2)
    df_vgg_2.to_csv('df_vgg_contrast.csv')
    ...
    model = tf.keras.models.load_model("model_vgg_contrast")
    test_age = []
    test_density = []
    for i in range(len(test_X)):
        index = df.index[df['Path'] == test_X['Path'].iloc[i]].tolist()[0]
    ...
    print('Test loss:', eval_metrics[0])
    print('Test accuracy:', eval_metrics[1])
    # df_vgg_2 = pd.read_csv('df_vgg_data2.csv')
