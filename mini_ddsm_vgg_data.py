...
if not model_saved:
    ...
    plt.savefig('vgg_data_accuracy.png')
    plt.clf()
    model.save('model_vgg_data')
    ...
    # Print the dataframe
    print(df_vgg_2)
    df_vgg_2.to_csv('df_vgg_data.csv')
    ...
    model = tf.keras.models.load_model("model_vgg_data")
    # for i in range(len(test_X)):
    #    path = test_X['Path'].iloc[i]
    #    img = load_img(path, target_size=(224, 224))
    #    img_arr = img_to_array(img)
    #    img_arr = img_arr / 255.0
    #    img_arr = np.expand_dims(img_arr, axis=0)
    #    print(np.argmax(model.predict(img_arr)), test_X['Label'].iloc[i])
    # Get all age/density values for the test set
    test_age = []
    test_density = []
    for i in range(len(test_X)):
    ...
    # df_vgg_2 = pd.read_csv('df_vgg_data.csv')
