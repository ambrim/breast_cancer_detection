...
if not model_saved:
  ...
  df_cnn = pd.DataFrame({'Path': df_read['Path'], 'Label': df_read['Label'], 'Prediction': predicted_labels, 'Age': df_read['Age'], 'Density': df_read['Density']})
  # Print the dataframe
  print(df_cnn)
  ...
  print("Test Accuracy: " + str(sum(df_cnn['Label'] == df_cnn['Prediction']) / len(df_cnn)))
  print(get_accuracies_by_age(df_cnn))
  print(get_accuracies_by_density(df_cnn))