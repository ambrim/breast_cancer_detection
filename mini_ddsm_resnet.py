...
if not model_saved:
    ...
    # Get the Resnet model with pre-trained weights
    ...
    # Create a new Sequential model and add the Resnet model to it
    model = Sequential()
    ...
    df_resnet = pd.DataFrame({'Path': df_read['Path'], 'Label': df_read['Label'], 'Prediction': predicted_labels, 'Age': df_read['Age'], 'Density': df_read['Density']})
    # Print the dataframe
    print(df_resnet)
    df_resnet.to_csv('df_resnet.csv')

    print(get_accuracies_by_age(df_resnet))
    print(get_accuracies_by_density(df_resnet))
    ...
    df_resnet = pd.read_csv('df_resnet.csv')
    print("Test Accuracy: " + str(sum(df_resnet['Label'] == df_resnet['Prediction']) / len(df_resnet)))
    print(get_accuracies_by_age(df_resnet))
    print(get_accuracies_by_density(df_resnet))